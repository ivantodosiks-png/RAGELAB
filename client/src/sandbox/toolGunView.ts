...
export class ToolGunView {
  ...
  kick(): void {
    this.recoil = Math.min(this.recoil + 0.045, 0.12);
  }
  ...
  update(dt: number, speedRatio: number, grounded: boolean, crouching: boolean): void {
    ...
    this.equip = lerp(this.equip, 1, 1 - Math.exp(-10 * dt));
    ...
    const bob = Math.sin(this.bobPhase) * 0.012 * Math.min(1, speedRatio);
    this.holder.position.set(
      this.hip.x + this.swayOffset.x + (crouching ? 0.01 : 0),
      this.hip.y + this.swayOffset.y + bob - (1 - this.equip) * 0.22 - (crouching ? 0.015 : 0),
      this.hip.z + this.recoil,
    );
    this.holder.rotation.set(-this.swayOffset.y * 0.4 + this.recoil * 0.8, this.swayOffset.x * 0.5, 0);
  }
