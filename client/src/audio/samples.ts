...
/**
 * Recorded one-shots. Most live in `client/public/sounds`; the AWM shot is
 * loaded from repo `assets/awm` so we do not duplicate the file.
 */
...
export function isRecordedGunshot(key: string): boolean {
  return key === 'pistol' || key === 'rifle' || key === 'shotgun' || key === 'sniper';
}
